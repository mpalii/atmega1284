...
for section in "${MEMORY_SECTIONS[@]}"; do
    OUTPUT_FILE="$OUTPUT_DIR/${section}_dump"
    echo "Reading '$section'..."
    if ! $AVRDUDE -c "$CONNECTOR" -p "$PART_NUMBER" -B "$BAUDRATE" -v -U "$section:r:$OUTPUT_FILE:$FORMAT"; then
