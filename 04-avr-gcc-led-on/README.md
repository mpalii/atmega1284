...
- To check connection with the AVR controller:  
 `avrdude -c avrispmkII -p m1284`  


---

### Led on (built in led port A, pin 0)

- Create the 'firmware.bin' file: `touch firmware.bin`.  
- With **hexedit** add the following instructions: **08 9A FF CF** (static light)  
- Where:  
  - **08 9a** - sbi (DDRA 0x01, pin 0) - 1001 1010 0000 1000 - 9a 08  
  - **ff cf** - rjmp (stay on the current instruction (-1 instruction pointer)) - 1100 1111 1111 1111 - cf ff  
- Print flash dump to console in HEX:  
`avrdude -c avrispmkII -p m1284 -U flash:r:-:h`  

---

### avr-gcc, blank 'main.c'
...
- To upload the firmware to the flash memory:  
 `avrdude -c avrispmkII -p m1284 -U flash:w:./firmware.elf:e`  
...
```assembly
  ldi   r28, 0xFF       ; Initialize stack pointer address (low byte)
  ldi   r29, 0x40       ; Initialize stack pointer address (high byte)
...
- ldi - Loads an 8-bit constant directly to register 16 to 31 - 1 cycle;  
- sts - Stores one byte from a Register to the data space (Register File, I/O memory, and SRAM) - 2 cycles;  
- out - Stores data from register Rr in the Register File to I/O space - 1 cycle;  
- sbi - Sets a specified bit in an I/O Register - 2 cycles;  
- lds - Loads one byte from the data space to a register (Register File, I/O memory, and SRAM) - 2 cycles;  
- ori - Performs the logical OR between the contents of register Rd and a constant - 1 cycle.  
