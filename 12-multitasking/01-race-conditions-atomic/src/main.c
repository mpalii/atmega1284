...
void init_io_pins(void)
{
    ...
    DDRD |= _BV(DDRD0) | _BV(DDRD1) | _BV(DDRD2) | _BV(DDRD3) | _BV(DDRD4);
}
...
void init_timer_1(void)
{
    ...
    TCCR1B = _BV(WGM12) | _BV(CS10);  // CTC mode and clk/1 (From prescaler)
	
    // OCR1A – Timer/Counter1 Output Compare Register channel A
    OCR1A =	0;                        //set the compare reg to 1 time ticks (zero based)
}
