...
/*******************************************************************************/
/* Set up timer 2 for toggling OC2A on Compare Match - PD7                     */
/* (256 (prescaler) * 100 (OCR2A + 1)) / 8_000_000 MHz = 0.0032Sec = 3.2mSec   */
/*******************************************************************************/
void init_timer_2(void)
{
    // TCCR2A – Timer/Counter2 Control Register channel A
    TCCR2A = _BV(COM2A0) | _BV(WGM21);  // CTC mode and Toggle OC2A on Compare Match.

    // TCCR2B – Timer/Counter2 Control Register channel B
    TCCR2B = _BV(CS22) | _BV(CS21);     // clk/256 (From prescaler)
	
    // OCR1A – Timer/Counter1 Output Compare Register channel A
    OCR2A =	249;                         // Set the compare reg to 1 time ticks (zero based)
}
...
void task_1(void)
{
    ...
    // frequency
    uint16_t f = (uint16_t) (F_CPU / timer_1_value);
    // uint16_t f = 1000;

    // Render frequency
    // sn74hc595_shift((uint16_t) timer_1_value);
    sn74hc595_shift(f);
}
